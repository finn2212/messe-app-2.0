  ...
  <div class="mt-8">
    ...
    <div v-else class="mt-4 bg-white shadow sm:rounded-lg p-4">
      <h2 class="text-lg font-semibold text-gray-900 mb-4">
        ...
        <span v-if="activeMesseId"> ({{ getMesseName() }}) </span>
      </h2>
        ...
        <ul>
          ...
          >
            ...
            >
              ...
              />
              <span
                >{{ answerText }}: <strong>{{ count }}</strong> mal
                gewählt</span
              >
            </div>
            <!-- Else just the text -->
...
// CSV Export for Quiz Statistics
function exportCsv() {
  // Erzeuge Zeilen: Eine Zeile pro Frage und Antwortoption
  const rows = [];
  for (const question of questions.value) {
    for (const option of question.options) {
      const count = quizStats.value[question.question]
        ? quizStats.value[question.question][option.text] || 0
        : 0;
      rows.push({
  ...
  const filename = `Quizz_Erbenisse_${getMesseTitle()}.csv`;
  CsvExportHelper.exportToCsv(rows, filename);
}

function getMesseTitle(): string {
  const messe = props.messen.find((m) => m.id === props.activeMesseId);
  return messe?.name || "Alle_Werte";
}
</script>
